...
try:
    ...
    col = {"country":country, "zip code":zip_code, "user email":user_email, "lat":lat, "long":long}
...
humidity_om = data['main']['humidity']
...
# Call the 'WeatherBit' API for additional metrics 
# WB API Info: 'https://www.weatherbit.io/api/weather-current'

r_wb = requests.get('https://api.weatherbit.io/v2.0/current', 
                        params={'postal_code':zip_code, 'units': 'I', 
                        'key':'c77fab04ef134a0183db546196371307'})
...
# Call the 'StormGlass' API for additional metrics
# SG API Info: 'https://docs.stormglass.io/#/'

start = arrow.now()
end= arrow.now()

r_sg = requests.get('https://api.stormglass.io/v2/weather/point',
            params={'lat': lat,'lng': long,'params': ','.join(['cloudCover', 'humidity']),
                        'start': start.to('UTC').timestamp, 'end': end.to('UTC').timestamp},
            headers={'Authorization':'2418e62c-d39b-11ea-bdeb-0242ac130002-2418e776-d39b-11ea-bdeb-0242ac130002'})

#Get required metrics from SG API
sg = r_sg.json()

for d in sg['hours']:
    clouds = d['cloudCover']['noaa']
    humidity_sg = d['humidity']['noaa']

#avg humidity
humidity = round((humidity_om + humidity_sg) / 2, 2)
...
def get_twitter():
    consumer_key = 'Ib0AijBXEBI02F16lr02nQxoQ'
    consumer_secret = 'pEKqQyDO0MORzLD5TLfY3UfLYZYQlch1aubaZHIprOS1pVLN6B'
    access_token = '95816074-bCPT6TYbG2CXO8Sn2Zp68NucBv3QJAMFt6m4EnEec'
    access_token_secret = 'wiDIySli13UmJY2B1Dh5Z5Y7FBaVoSAnjYYARWH75CYfx'
